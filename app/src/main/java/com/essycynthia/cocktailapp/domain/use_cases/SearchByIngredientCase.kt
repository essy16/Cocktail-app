...
class SearchByIngredientCase @Inject constructor(private val repository: CocktailRepository) {
    operator fun invoke(ingredient:String): Flow<Resource<List<Drink>>> = flow {
        try {
            emit(Resource.Loading())
            val book=repository.searchCocktailByIngredient(ingredient).drinks.map { it.toDrink() }
