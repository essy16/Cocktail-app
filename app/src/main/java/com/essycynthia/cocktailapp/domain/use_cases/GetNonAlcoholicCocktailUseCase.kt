...
class GetNonAlcoholicCocktailUseCase @Inject constructor(val repository: CocktailRepository) {
    operator fun invoke(): Flow<Resource<List<Drink>>> = flow {
        try {
            emit(Resource.Loading(null))
            val nonAlcoholicCocktails = repository.getNonAlcoholicCocktails().map { it.toDrink() }
            emit(Resource.Success(nonAlcoholicCocktails))
