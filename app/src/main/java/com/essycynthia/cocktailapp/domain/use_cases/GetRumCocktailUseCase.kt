...
class GetRumCocktailUseCase @Inject constructor(val repository: CocktailRepository) {
    operator fun invoke(): Flow<Resource<List<Drink>>> = flow {
        try {
            emit(Resource.Loading(null))
            val rumCocktails = repository.getRumCocktails().map { it.toDrink() }
            emit(Resource.Success(rumCocktails))
