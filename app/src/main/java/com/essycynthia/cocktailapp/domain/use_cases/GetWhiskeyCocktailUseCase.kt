...
class GetWhiskeyCocktailUseCase @Inject constructor(val repository: CocktailRepository) {
    operator fun invoke(): Flow<Resource<List<Drink>>> = flow {
        try {
            emit(Resource.Loading(null))
            val whiskeyCocktails = repository.getWhiskeyCocktails().map { it.toDrink() }
            emit(Resource.Success(whiskeyCocktails))
