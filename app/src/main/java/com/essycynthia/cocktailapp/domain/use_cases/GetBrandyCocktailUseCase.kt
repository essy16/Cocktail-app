...
class GetBrandyCocktailUseCase @Inject constructor(val repository: CocktailRepository) {
    operator fun invoke(): Flow<Resource<List<Drink>>> = flow {
        try {
            emit(Resource.Loading(null))
            val brandyCocktails = repository.getBrandyCocktails().map { it.toDrink() }
            emit(Resource.Success(brandyCocktails))
